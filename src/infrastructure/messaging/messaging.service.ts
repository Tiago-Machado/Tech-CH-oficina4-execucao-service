...
export class MessagingService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit(): Promise<void> { await this.conectar(); }

  async onModuleDestroy(): Promise<void> {
    try { await this.channel?.close(); await this.connection?.close(); } catch (_) {}
  }
  ...
  async publicar(routingKey: string, payload: object): Promise<void> {
    try {
      ...
      this.logger.error(`Falha ao publicar ${routingKey}`, err);
    }
  }

  async assinar(queue: string, routingKey: string, handler: (msg: any) => Promise<void>): Promise<void> {
    ...
    await this.channel!.consume(queue, async (msg) => {
      ...
      try {
        ...
        this.logger.error(`Erro na fila ${queue}`, err);
